...
## D-007 · 项目定位：工厂本身是产品，试点是陪练（第7轮）
- **时间**：2026-06-11
- **决策**：本项目的核心交付物是"AI 项目孵化工厂"这套系统本身（流水线 + 能力边界）；
  "讨债/催收"等试点项目仅作为压测工厂、暴露问题的"陪练沙包"，不作为正式开发目标。
- **角色**：老板=教练，Agent=陪练。重心是工厂的可用性与边界。
- **理由**：老板明确说明建厂目的是以后能反复用它开发其它项目。
- **状态**：生效中（已写入 HANDOFF 0.0）。

## D-008 · GLM 链路问题闭环 + 扩充本地模型（第7轮）
- **时间**：2026-06-11
- **GLM 定论**：所有"非流式不稳/被 fallback"现象的根因 = GLM_API_KEY 填错/未 export。
  填对 Key 并用 start-litellm.sh 启动后，diag-glm-v2 全绿（直连/网关、流式/非流式均正常）。
- **新增本地模型**（利用老板已有的 Ollama 模型）：
  - `local/coder` = qwen3-coder-next:q4_K_M（编程专用）
  - `local/embedding` = bge-m3（向量，首选）
  - `local/embedding-large` = qwen3-embedding:8b（向量备选）
- **状态**：配置已加，待真机 V-LocalModels 验证。
