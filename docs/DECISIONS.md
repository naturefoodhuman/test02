## Factory-Level ADRs (Root SSOT since 2026-06-16 Governance Audit)

Major architectural decisions for the FORGE Factory are now recorded as immutable ADRs in `docs/adr/`.

See:
- `docs/adr/README.md` (index)
- `docs/adr/ADR-001-langgraph-migration.md` through `ADR-007-memorystore-as-plan-comparison-ssot.md`

These supersede the early D-xxx notes below for v1.1.0+.

---

## Legacy Early Decisions (pre full governance)
...
- **状态**：核心图结构与平台层已落地，16 个新测试通过，CLI 已切换入口。（详见 `docs/adr/ADR-001`）
...
- **状态**：作为过渡实现保留，待 LangGraph 验证稳定后删除。（已被 ADR-001 正式取代）
