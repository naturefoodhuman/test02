...
## v1.2.9 里程碑记录（2026-06-20） — 真实 LLM 调用首次成功

**目标**：打通 Smart Proxy 流式调用长思考模型

### 压测结果（Mac M1 Max 真机）

| 方案            | 状态          | 耗时      | 分歧度 | 备注 |
|------------------|---------------|-----------|--------|------|
| default          | ✅ 成功        | 1132.5s   | 0.0    | 首次获得真实共识报告 |
| high-quality     | 未测试        | -         | -      | 需 API Key |
| all-local        | 未测试        | -         | -      | - |
| mtplx-hybrid     | 未测试        | -         | -      | - |

**关键突破**：
- 实现 SSE 流式直通 + 600s chunk 超时
- 自动拉起 MTPLX 模型
- 解决 400/404/超时问题
...
## v1.2.6 里程碑记录（2026-06-20）

**目标**：打通模型压测链路，解决 `ConfigurationError: 模型配置不一致`

### 压测结果（沙箱环境）

| 方案            | 状态     | 耗时   | 分歧度 | 备注 |
|------------------|----------|--------|--------|------|
| default          | ✅ 成功   | 0.1s   | 0.0    | 配置已打通 |
| high-quality     | ✅ 成功   | 0.2s   | 0.0    | 配置已打通 |
| all-local        | ✅ 成功   | 0.1s   | 0.0    | 配置已打通 |
| mtplx-hybrid     | ✅ 成功   | 0.1s   | 0.0    | 配置已打通 |
| full-check       | ✅ 已移除问题节点 | - | - | 不再报错 |

**关键修复**：
- 移除 `routing_plans.yaml` 中 `full-check` 方案的 `fast_classify: local-fast` 节点
- 配置交叉验证通过
- 所有方案模型引用一致性 100%
...
- 配置层完全健康
- LangGraph 执行流程正常
- 仅因缺少本地模型服务（Connection refused / 404）而无法获得真实 LLM 输出
- 下一步需在 Mac 真机启动 Smart Proxy + 模型服务后再次验证

**验收**：压测脚本已从“启动即失败” → “配置通过，进入执行阶段”