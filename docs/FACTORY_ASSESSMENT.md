...
| FB-11 | ⭐**专家系统/决策大脑**：领域专家(角色+知识库RAG+模型路由+更新)独立为工厂能力，多项目复用 | 领域项目智力上限受限于纯LLM | **高** | 已设计(expert-system-design.md)+模板就绪，待落地首个专家 |
| FB-10 | 缺『反封号取数』可插拔能力(代理/反检测浏览器Camoufox/拟人/熔断)+数据流水线(采集→清洗→结构化→缓存→供给) | 强风控平台取数能力弱 | 中 | 已调研(anti-ban-crawling-strategy.md)，落地按需 |
...
## 二·六、第21轮工厂升级
- ✅ **R4 LLM 遥测**：新增工厂 Pattern `_factory/patterns/llm-telemetry`(事件+耗时+实时计时器+JSONL，供模型对比)。
- ✅ **合规判定 v2**(项目级，可上升工厂)：否定语境识别(修复『绝不威胁』误判)+严重度分级+结构化整改反馈+递归重生成。
- 📌 经验：**关键词匹配做合规会假阳性**，必须否定语境感知；不合规要给可回传 LLM 的结构化理由(递归整改)。

## 二·七、第23轮方法论修正(老板指出)
- ❗**自查缺陷**：此前多个决策(爬虫/工具/模型选型)调研不够穷尽 → 新增规则 R5(调研穷尽)、R6(缺知识求助不假装)。
- ✅ 补做开源模型全景调研(R5)：发现 Qwen3.5系列/DeepSeek-V3.2/GLM-Z1/Devstral/GPT-OSS 等本机可跑且部分更强 → 写入 expert-system-design 供专家选脑。
- ✅ 新增**专家系统(FB-11)**：工厂决策大脑，LLM+本地领域知识沉淀，多项目复用。
- ✅ 新增 **FACTORY_OPERATIONS.md**：工厂运转 SOP + 外源知识获取分工。
