...
- **AI Parenting（从零重建）项目独立 CHANGELOG**：`projects/AI-Parenting/docs/CHANGELOG.md`（与 `projects/AI-Parenting-Copilot/` 隔离，本项目变更见该路径，不在此重复记录）。
- **AI Parenting 进度**：APC-T001~T012 完成（Milestone 1 地基 + Milestone 2 事件溯源与同步），285 passed；最新见 `projects/AI-Parenting/docs/PROJECT_STATE.md`。
...
### 需求变动
- **GLM 慢速无 fallback 工作档验证**：用户同意新增 `forge-nim-glm-slow-smoke` 与 `forge-nim-vscode-glm-slow-watch` 两个自动化目标。
- **新增 `glm-slow` profile**：保持 `NIM_PROXY_ENABLE_FALLBACK=0`，设置 `NIM_PROXY_READ_TIMEOUT_SECONDS=360`、`NIM_PROXY_REQUEST_WALL_TIMEOUT_SECONDS=600`、`FORGE_REMOTE_MAX_CONCURRENCY=1`、`NIM_PROXY_PER_KEY_CONCURRENCY=1`、`FORGE_CTX_SOFT_TOKENS=12000`、`FORGE_CTX_KEEP_RECENT_TURNS=4`、`FORGE_CTX_TRUNC_TOOL_RESULT_CHARS=800`。
- **测试行为说明**：`forge-nim-glm-slow-smoke` 会跑 4010/4000 最小 non-stream smoke；`forge-nim-vscode-glm-slow-watch` 只做 VS Code 固定窗口观测，不预跑 curl smoke，避免测试前额外等待。
...
# 7 passed
python3 -m py_compile scripts/diagnostics/forge_nim_diagnostic.py
make -n forge-nim-glm-slow-smoke
make -n forge-nim-vscode-glm-slow-watch
make docs-check
...
### 需求变动
- **artifact push 失败修复**：用户的实验 A 探针已跑到 artifact push 阶段，但 `git commit` 在 orphan diagnostics worktree 中返回 1，导致整个 `make forge-nim-timeout-a` 失败。
- **push 阶段容错**：诊断脚本现在使用 `git add -f` 与 `git commit --no-verify`，并把 push/commit stdout/stderr 写入 `push_sanitized_artifact.log`。即使 push 失败，也会保留 `DIAG_OUTPUT_DIR` / `DIAG_TARBALL` / `CURL_RESULTS`，不让已完成探针结果丢失。
- **已有产物补推**：新增 `make forge-nim-push-existing`，可对已经生成的 `/private/tmp/forge_nim_diag_*` 目录补推脱敏 artifact 分支，无需重跑 300 秒 timeout 探针。
...
python3 -m pytest _infra/network/tests/unit/test_forge_nim_diagnostic.py -q
python3 -m py_compile scripts/diagnostics/forge_nim_diagnostic.py
make -n forge-nim-push-existing
```
...
### 需求变动
- **自动诊断启动卡住修复**：用户执行 `make forge-nim-timeout-a` 超过 6 分钟卡在 `Restarting 4000/4010 via scripts/forge-start.sh ...`。根因是诊断脚本复用了 `forge-start.sh`，该脚本会做全量本地模型冷启动自检；NIM timeout 实验不需要这一步。
- **快速重启模式**：`forge_nim_diagnostic.py` 新增默认 `--restart-mode fast`，只重启 4010 NIM sidecar 与 4000 Smart Proxy，跳过 `forge-start.sh` 全量本地模型自检。保留 `--restart-mode full` 作为显式选项。
- **实验 A 仍不启用 fallback**：`timeout-a` profile 明确设置 `FORGE_USE_NIM_PROXY=1` 与 `NIM_PROXY_ENABLE_FALLBACK=0`。
...
### 文件影响
- 新增：`scripts/diagnostics/env_config_audit.py`
- 新增：`_infra/network/tests/unit/test_env_config_audit.py`
- 新增：`docs/ENV_CONFIG_GUIDE.md`
- 修改：`_infra/start-litellm.sh`
- 修改：`Makefile`
- 修改：`docs/CHANGELOG.md`
...
### 需求变动
- **AI Parenting Copilot Quick Record/Copilot 继续推进**：Android native Quick Record 增加 Copilot 文本解析并保存本地 pending，网络/API 不可用时用本地 deterministic fallback 保存 pending；RN/TS 增加 Copilot flow helper，DB API smoke 扩展 Copilot query/confirm、FamilyMemory confirm audit 与 P0 Rule Evaluation。

### 文件影响
- 新增：`projects/AI-Parenting-Copilot/android/src/features/quick_record/copilotFlow.ts`
- 修改：`projects/AI-Parenting-Copilot/android/android/app/src/main/java/com/aiparentingcopilot/QuickRecordActivity.kt`
- 修改：`projects/AI-Parenting-Copilot/tests/integration/test_api_db_runtime.py`
- 修改：`projects/AI-Parenting-Copilot/tests/test_android_features.py`
...
# healthz ok; dev-mock DB; MQTT offline expected in sandbox
```
...
### 需求变动
- **AI Parenting Copilot mmWave 继续推进**：新增 shared ingest service、live MQTT worker、run_mmwave_worker CLI 与 Make target。

### 文件影响
- 新增：`projects/AI-Parenting-Copilot/server/app/mmwave/ingest_service.py`
- 新增：`projects/AI-Parenting-Copilot/server/app/mmwave/worker.py`
- 新增：`projects/AI-Parenting-Copilot/server/scripts/run_mmwave_worker.py`
- 新增：`projects/AI-Parenting-Copilot/tests/test_mmwave_ingest_service.py`
- 修改：Makefile / project docs / root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Camera shadow 推进**：新增 camera fusion evaluate API，连接 fusion decision、clip plan 与 camera_event storage。

### 文件影响
- 修改：`projects/AI-Parenting-Copilot/server/app/camera/api/routes.py`
- 修改：camera tests / API DB smoke / project docs / root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Camera shadow 推进**：新增 composite camera shadow evaluate API，一次性组合 fusion decision、clip plan、camera_event 与 optional VLM shadow。

### 文件影响
- 修改：camera API, camera shadow tests, project docs, root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Camera/VLM 推进**：新增 Camera VLM shadow dispatch API，VLM 仍保持 shadow-mode 且只使用注入 Model Gateway client。
...
### 需求变动
- **AI Parenting Copilot Copilot 闭环推进**：新增 record candidate confirm 与 family memory confirm endpoints，候选输出可落库并审计。

### 文件影响
- 修改：orchestrator API/routes, orchestrator tests, API DB smoke, project docs, root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Memory/FamilyKnowledge 推进**：新增 FamilyKnowledge API/DB repo/audit，为 FamilyMemory Copilot 持久化补齐服务端路径。

### 文件影响
- 新增：server memory family_knowledge repository/API/tests
- 修改：FastAPI main、API DB smoke、project docs、root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Sync 可观测性推进**：新增 SyncState heartbeat API 与 Android pending drain heartbeat 上报。

### 文件影响
- 新增：server sync state repos/API/tests
- 修改：Android pending_sync_drain, FastAPI main, API DB smoke, project docs, root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Android native 继续推进**：AlertCenter native screen 增加 feedback 提交，SleepSession native screen 增加 ROI 保存与 camera events 查询。

### 文件影响
- 修改：Android NativeApiClient / AlertCenterActivity / SleepSessionActivity / static tests / project docs / root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Android 继续推进**：新增 Android JobScheduler background drain、API Settings screen、BootReceiver，推进离线记录/ack 后台补传。
- **AI Parenting Copilot mmWave API 增强**：新增按 device 查询 sensor events。

### 文件影响
- 新增：Android ApiSettings/BackgroundDrain/BootReceiver native files
- 修改：Android manifest/application/main, mmWave API, tests, project docs, root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Android native 继续推进**：新增 NativeApiClient、PendingSyncDrainer、AlertAckDrainer，让 native PendingEventsActivity 可直接 drain 本地 pending event 到服务端。

### 文件影响
- 新增：`projects/AI-Parenting-Copilot/android/android/app/src/main/java/com/aiparentingcopilot/NativeApiClient.kt`
- 新增：`projects/AI-Parenting-Copilot/android/android/app/src/main/java/com/aiparentingcopilot/PendingSyncDrainer.kt`
- 新增：`projects/AI-Parenting-Copilot/android/android/app/src/main/java/com/aiparentingcopilot/AlertAckDrainer.kt`
- 修改：Android native action receiver / pending screen / static tests / project docs / root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Android 继续推进**：新增 pending local event drain 与 alert ack action drain，推进 Android 离线记录/告警确认闭环。

### 文件影响
- 新增：`projects/AI-Parenting-Copilot/android/src/sync/pending_sync_drain.ts`
- 新增：`projects/AI-Parenting-Copilot/android/src/notification/ack_drain.ts`
...
### 需求变动
- **AI Parenting Copilot Camera/mmWave 继续推进**：新增 mmWave frame ingest API、Camera event create/list API，并扩展 API DB smoke。

### 文件影响
- 新增：`projects/AI-Parenting-Copilot/server/app/mmwave/api/routes.py`
- 新增：`projects/AI-Parenting-Copilot/tests/test_mmwave_api.py`
- 修改：Camera API、FastAPI main、API DB smoke、project docs、root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Camera/mmWave 推进**：新增 sensor_event/camera_event SQLAlchemy repositories，并扩展 DB repository integration smoke。

### 文件影响
- 新增：`projects/AI-Parenting-Copilot/server/app/mmwave/sqlalchemy_sensor_event_repo.py`
- 新增：`projects/AI-Parenting-Copilot/server/app/camera/sqlalchemy_camera_event_repo.py`
- 修改：DB repository integration tests / static tests / project docs / root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Scheduler/Backup 推进**：新增 PeriodicSchedulerWorker 与 Backup restore drill planner/runbook，推进 `APC-T036/T044`。

### 文件影响
- 新增：`projects/AI-Parenting-Copilot/server/app/scheduler/worker.py`
- 新增：`projects/AI-Parenting-Copilot/server/app/backup/restore_drill.py`
- 新增：`projects/AI-Parenting-Copilot/docs/BACKUP_RESTORE_RUNBOOK.md`
- 新增：`projects/AI-Parenting-Copilot/tests/test_scheduler_worker.py`
- 修改：server main / Makefile / backup tests / project docs / root CHANGELOG
...
### 需求变动
- **AI Parenting Copilot Sleep/Media/Export 推进**：新增 Sleep Session DB repo/API audit、Media DB persistence/audit、Export API/audit，并扩展 API DB smoke。

### 文件影响
- 新增：`projects/AI-Parenting-Copilot/server/app/export/api/routes.py`
- 新增：`projects/AI-Parenting-Copilot/tests/test_export_api.py`
- 修改：camera/media/main/API DB smoke/project docs/root CHANGELOG
