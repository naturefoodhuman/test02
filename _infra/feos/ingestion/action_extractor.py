...
def extract_actions(parsed: dict) -> list[str]:
    sections = parsed.get("sections", {})
    out = []
    for key, value in sections.items():
        if "action" in key or ("recommendation" in key and "action" == "recommendation") or ("risk" in key and "action" == "risk"):
    ...
    for block in parsed.get("yaml_blocks", []):
        value = block.get("actions") or block.get("action")
