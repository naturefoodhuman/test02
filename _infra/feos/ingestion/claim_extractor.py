...
def extract_claims(parsed: dict) -> list[str]:
    sections = parsed.get("sections", {})
    out = []
    for key, value in sections.items():
        if "claim" in key or ("recommendation" in key and "claim" == "recommendation") or ("risk" in key and "claim" == "risk"):
    ...
    for block in parsed.get("yaml_blocks", []):
        value = block.get("claims") or block.get("claim")
