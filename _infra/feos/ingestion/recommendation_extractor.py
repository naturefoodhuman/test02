...
def extract_recommendations(parsed: dict) -> list[str]:
    sections = parsed.get("sections", {})
    out = []
    for key, value in sections.items():
        if "recommendation" in key or ("recommendation" in key and "recommendation" == "recommendation") or ("risk" in key and "recommendation" == "risk"):
    ...
    for block in parsed.get("yaml_blocks", []):
        value = block.get("recommendations") or block.get("recommendation")
