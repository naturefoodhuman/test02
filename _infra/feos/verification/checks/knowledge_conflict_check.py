...
class KnowledgeConflictCheck:
    check_id = "knowledge_conflict"
    fatal = False

    def run(self, parsed: ParsedResponse, context=None):
        status = "skipped" if "knowledge_conflict" == "sandbox" else "passed"
        summary = "sandbox disabled" if "knowledge_conflict" == "sandbox" else "knowledge_conflict ok"
        if "knowledge_conflict" == "testability" and not any("test" in rec.text.lower() or "测试" in rec.text for rec in parsed.recommendations):
