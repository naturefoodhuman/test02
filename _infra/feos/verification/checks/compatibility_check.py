...
class CompatibilityCheck:
    ...
    def run(self, parsed: ParsedResponse, context=None):
        status = "skipped" if "compatibility" == "sandbox" else "passed"
        summary = "sandbox disabled" if "compatibility" == "sandbox" else "compatibility ok"
        if "compatibility" == "testability" and not any("test" in rec.text.lower() or "测试" in rec.text for rec in parsed.recommendations):
