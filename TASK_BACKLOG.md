...
- **输出**: 扫描脚本
- **涉及文件**:
  - 新建：`scripts/scan-mcp.sh`
  - 新建：`src/mcp/scanner.py`
- **实现要求**:
  - 调用 `mcp-scan scan`
  - 解析输出 JSON
  - 检测：tool poisoning、rug pull、schema 变化、PII
  - 失败时返回非 0 退出码
- **测试要求**:
  - 单元测试：解析 mcp-scan 输出
  - 集成测试：实际扫描 playwright MCP
- **验收标准**:
  - 脚本可执行
  - 检测到问题时报告清晰
- **DoD**:
...
- **涉及文件**:
  - 新建：`src/mcp/guard.py`
  - 新建：`src/mcp/models.py`
- **实现要求**:
  - 定义 `MCPToolCall`、`MCPToolResult`、`GuardDecision` 模型
...
- **涉及文件**: 新建 `.mcp.json.research`
- **实现要求**:
  - 允许：searxng / crawl4ai / playwright-public
  - 禁止：任意 shell / private profile
  - 本地路径引用
- **测试要求**: JSON 合法
- **验收标准**: 文件合法
- **DoD**:
...
- **输出**: 测试用例
- **涉及文件**:
  - 新建：`tests/security/test_canary_e2e.py`
- **实现要求**:
...
| M2 | E3-C2 | E3-C2-S1-T1 | [ ] | | |
| M2 | E3-C2 | E3-C2-S1-T2 | [ ] | | |
| M2 | E3-C3 | E3-C3-S1-T1 | [ ] | | |
| M2 | E3-C3 | E3-C3-S1-T2 | [ ] | | |
