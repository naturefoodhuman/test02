...
**当前任务状态**：用户本机 `api-db-smoke-test` 复验通过后 `APC-T037/T042/T043` 已 DONE；Android build 已通过；Scheduler create_alert reminder bridge、Camera ISAPI/Fregata HTTP bridge、Rule Review Packet generator、launchd static validator、backup manifest verifier、red alert escalation report、firmware static preflight、mmWave replay report 与 Android/PowerSync E2E contract report 已完成；APC-T046/T047/T048/T049/T050/T051/T052/T053/T054/T055/T056/T057 已 DONE；`APC-T022/T023/T030/T036/T038/T039/T048` 继续推进但仍等待生产规则审查/device/PowerSync 等复验；累计 DONE 以 `docs/TASK_BACKLOG.md` 顶部状态行为准。
...
### 继续开发进展（DB smoke assertion fix / Android build accepted）
...
- `APC-T047`、`APC-T052`、`APC-T054`、`APC-T056`、`APC-T057` 已基于合同报告、E2E substitutes、用户 Android build/API DB smoke 和自动化回归一次性标记 DONE。
- 新增 Android notification contract report 与 deployment readiness report；Android/PowerSync/Red Alert E2E substitute 已覆盖主要代码合同。
- 真实 RTSP/mmWave/FCM/NAS/7-night soak 仍作为剩余硬件/发布硬化任务跟踪，不混入已完成代码任务。

### 状态接受（APC-T050/T051/T053）

- `APC-T050` Timeline、`APC-T051` Alert Center、`APC-T053` Sleep Session UI 已基于用户 `assembleDebug`、static tests、Android E2E contract report 标记 DONE。
- Timeline/Alert/Sleep 的真实跨端设备 E2E 仍集中由 `APC-T056/T057` 跟踪，不再阻塞单页面代码任务。

### 状态接受（APC-T046/T048/T049/T055）

- `APC-T046` Android Auth、`APC-T048` Quick Record、`APC-T049` Today 首页已基于用户 `assembleDebug`、static/API tests 和 Android E2E contract report 标记 DONE。
- `APC-T055` Dev Fixtures 已基于 fake channels、mock mmWave publisher、radar fixtures、mmWave replay report、安全回归与 E2E substitutes 标记 DONE。
- 真实 Android/PowerSync 端到端仍由 `APC-T056` 跟踪，避免把设备验收混入单功能代码任务。

### 继续开发进展（mmWave replay / Android E2E contract reports）

- 新增 mmWave fixture replay report 和 `make mmwave-replay`，可用 JSONL fixture 验证 parser/mapper/ObservationEvent contract。
- 新增 Android/PowerSync E2E contract report 和 `make android-e2e-contract`，静态验证 Android schema/drain/heartbeat/offline fallback/Today pending visibility，并用 server sync contract validator 校验 sample event。

### 继续开发进展（ESP32C6 firmware static preflight）

- 修复 ESP32C6 firmware mock MQTT payload JSON 字符串转义，并增加串口输出，便于本机调试。
- 新增 firmware preflight 脚本/Make target，静态校验 PlatformIO board、PubSubClient dependency、MQTT topic、placeholder secrets 与 payload JSON shape。
...
- 新增 backup manifest verifier 与 `make backup-verify-dry-run`，可在真实 NAS/restore drill 前验证 manifest、dump/media archive path、tar 安全路径和归档 scope。
- verifier 输出 `pg_restore --list`、`pg_restore --clean`、`tar -tzf` 下一步命令，便于用户 Mac 本地执行真实恢复演练。

### 继续开发进展（launchd static validator）

- 新增 launchd plist validator 与 `make launchd-validate`，校验 plist XML、ProgramArguments、runtime/logs 日志路径等部署约束。
- `com.parenting.backup.plist` 日志路径已从 `/tmp` 改为 `runtime/logs/`，符合项目 runbook 要求。

### 继续开发进展（Rule Review Packet generator）

- 新增 Rule Review Packet builder/CLI/Make target，汇总 rule pack hash、source、rule ids、golden-case pass/fail 与 pending human-review blockers。
- Packet 明确 Vaccine 官方 CN 免疫规划审查与 Growth 完整 WHO LMS 表仍是生产 blocker，避免把 dev fixture 误标为生产批准。

### 继续开发进展（Camera ISAPI/Fregata HTTP bridge）

- `ISAPIClient` 现在可调用真实 `/ISAPI/System/status` 做 health probe，并支持 fake HTTP client 注入测试。
- `FregataBridge` 现在可 POST snapshot bytes 到外部 Fregata endpoint，并把 events/detections 标准化为 shadow events；异常保持 shadow/offline，不直接触发红橙告警。

### 继续开发进展（Scheduler reminder alert bridge / Sleep-Media-Export accepted）

- `APC-T037` Sleep Session、`APC-T042` Media Storage、`APC-T043` Export 已根据用户本机 `api-db-smoke-test` 复验通过标记 DONE。
- Scheduler API trigger/trigger-all 新增 `create_alert` 可选参数，能够把带 `alert_level` 的 job result 转为 `scheduler.{job}` AlertStore 提醒；实际投递仍走 Notification Orchestrator。
...
- Native `PendingSyncDrainer` 上传 pending events 时逐条捕获网络/API 异常，失败事件保持 pending，不影响后续事件 drain。
- drain 结束后上报 `/api/v1/sync/heartbeat`，同步 native fallback pending count 到服务端 sync_state；heartbeat 失败不影响本地可靠性。
